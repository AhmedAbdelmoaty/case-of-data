...
export const ResultScreen = ({ onNavigate }: ResultScreenProps) => {
  ...
  const trackLabel = useMemo(() => {
    if (!state.trackEntered && !state.hasUsedRecovery) return "مسار نظيف — مشيت على الـ spine للنهاية";
    if (!state.trackEntered && state.hasUsedRecovery) return "صحّحت بعد فرصة الـ recovery";
    return `دخلت في مسار ${state.trackEntered} ووصلت لنهايته`;
  }, [state.trackEntered, state.hasUsedRecovery]);
